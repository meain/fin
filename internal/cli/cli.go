...
func printDoctor(cfg *config.Config) {
	...
	for _, n := range provNames {
		...
		row(n, fmt.Sprintf("%-40s %-30s %s", p.BaseURL, dim(p.APIKeyEnv), keyStatus))
	}
	fmt.Println()
	...
	for _, tl := range tools {
		...
		approvalColor := dim(approvalMode)
		if approvalMode == "confirm" {
			approvalColor = warn(approvalMode)
		} else if approvalMode == "deny" {
			approvalColor = render.Red + approvalMode + render.Reset
		}
		row(tl.Name(), fmt.Sprintf("%-28s %s", approvalColor, dim(shortDesc(tl.Description(), 70))))
	}
	fmt.Println()
