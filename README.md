...
**Forking** (`-fork`) creates a new session with `previous_session` pointing to the origin and copies all messages into it. The fork gets a fresh title generated from the fork prompt. In `fin -sessions` (TTY), forks appear grouped under their parent with `↳` indentation; JSON output includes a `parent_id` field. Exporting a fork gives the complete transcript — the full history is already present in the fork's messages.
